...
var dbHandle , getDbCollectionHandle , ObjectId , pubsub;
...
var handles = {
    ...
    login : function(token,pwd,args, callback) {
        ...
        //if(!userCollection){
		/*
            getDbCollectionHandler('smr_RemoteUser',function(err,collection){
                userCollection = userCollection || collection;
                handles.login.apply({},args);
            });
		*/
		/*
			pubsub.subscribe('pubremote', token, pwd, function(collection){	
				userCollection=collection.find({'username':token});
				handles.login.apply({},args);
			});
		*/
		//	return;
		...
		var userCollection=new serverCollection.create('smr_RemoteUser');

		userCollection.extfind('pubremote',token,pwd,function(e){
            ...
            callback(null,{id:user.username,info:info});
